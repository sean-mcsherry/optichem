...
class model:
    ...
    def auto_peak_find(self):
        ...
        if self.df.empty:
            ...
            self.df_old = self.df
            self.df = []
            for jj in range(len(self.df_old['w0'])):
                if not np.min(self.wL)<= self.df_old['w0'][jj]<=np.max(self.wL):
                    self.df_old = self.df_old.drop(jj)
                self.df = self.df_old
                #self.df_old = []
            for kk in range(len(self.df_new['w0'])):
                if not np.min(self.wL_old)<= self.df_new['w0'][kk] <=np.max(self.wL_old):
                    self.df = self.df.append(self.df_new.iloc[kk])
                                                               
        self.df = self.df.sort_values(['height'], ascending=False)
        self.df = self.df.reset_index(drop=True) 
        self.df_og = self.df
        
    def _init_plot(self):
        self._figure = plt.figure(num='Add (left click), delete (right click), or drag (left click + drag) new peaks',figsize=(9,8))
        ...
        plt.plot(self.wL,self.A, lw = 3, color = 'k')
        axes_2.grid(which="both")
        self._axes_2 = axes_2
        x, y = zip(*self._points.items())
        self._line, = self._axes_2.plot(x, y, "r", marker="+", markersize=10, linestyle='None')       
        ...
        def onrangeselect(xmin, xmax):
            ...
            self.set_range(self.x_lim_min, self.x_lim_max)
   ...
   #--- initate solver ---     
    ...
    def _remove_point(self, x, _):
        if x in self._points:
            self._points.pop(x)
            self.df = self.df[self.df.w0 != x]
            self.df = self.df.reset_index(drop=True)
    
    def closest_point(self, event):
        distance_threshold = 0.004*self.unit_conversion(10,'um', self.output_units)
        nearest_point = None
    ...
    def _on_click(self, event):
        # left click
        if event.button == 1 and event.inaxes in [self._axes_2]:
            point = self.closest_point(event)
            if point:
            ...
            self._update_plot()
        # right click
        elif event.button == 3 and event.inaxes in [self._axes_2]:
            point = self.closest_point(event)
            if point:
    ...
    def _on_release(self, event):
        if event.button == 1 and event.inaxes in [self._axes_2] and self._dragging_point:
